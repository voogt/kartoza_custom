frappe.ready(() => {
	...
	function send_inquiry() {
		...
		frappe
			.call("erpnext.e_commerce.shopping_cart.cart.create_lead_for_item_inquiry", {
				lead: doc,
			...
			.then((r) => {
				if (r.message) {
					...
					frappe.msgprint({
						title: __('Success'),
						indicator: 'green',
						message: __('Your inquiry has been successfully sent.'),
					});
				}
			})
			.catch((err) => {
				frappe.throw({
					title: __('Error'),
					message: __('There was an issue sending your inquiry. Please try again later.'),
				});
			});
	}
