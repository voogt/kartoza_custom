...
def get_project_sla_overview_table():
    ...
    SELECT 
        ...
        '' as `billable_hours`,
        '' as `billable_hours_percent`,
        '' as `billable_hours_left`,
        tp.percent_complete as `actual_progress`,
        '' as `progress_status`,
        tp.expected_end_date as `due_date`,
        '' as `total_sales_amount_via_sales_order`,
    ...
    for arr in final_arr:
        project = arr["project"]
        timesheet_details = getTimesheetDetail(project)
        ...
        if actual_progress > arr['billable_hours_percent']:
            arr["progress_status"] = 'On Track'
        if actual_progress < (arr['billable_hours_percent'] + arr['billable_hours_percent'] * 0.10):
            arr["progress_status"] = 'Warning'
        if actual_progress >= (arr['billable_hours_percent'] + (arr['billable_hours_percent'] * 0.10)):
            arr["progress_status"] = 'Critical'
        
        sales_order_total = getAllSalesOrders(project) or 0
...
def get_task_drill_down_table():
    ...
    for arr in final_arr:
        task = arr["milestone"]
        timesheet_details = getTimesheetDetailTask(task)
